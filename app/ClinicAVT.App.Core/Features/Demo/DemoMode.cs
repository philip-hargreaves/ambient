...
/// <summary>A saved run to play back.</summary>
public sealed record DemoMaster(string SessionId, double AudioSeconds);

/// <summary>
/// In demo mode Record plays a saved run back instead of listening. It is a developer
    ...
    IReadOnlyList<DemoCase>? cases = null)
{
    ...
    /// <summary>
    /// Turning it on re-reads the masters, so a fresh recording needs no restart.
    /// </summary>
    public bool Enabled
    {
        get => _enabled;
