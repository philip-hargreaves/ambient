...
/// <summary>
/// The review of one consultation: the note and patient sheet behind one selector, the
/// guidelines, transcript and an opened document page behind another, or all five behind
/// one when narrow. The live screen shows it as the note streams; Sessions shows it for a
/// stored consultation. One instance per page, since an element has one parent.
/// </summary>
public sealed partial class ReviewSurfaceView : UserControl
{
        ...
        ConsultationViewModel consultation, PageView page)
    {
        ...
        // The note grows as it streams; the transcript follows it
        _note.SizeChanged += (_, _) => Fit();
    }

    /// <summary>
    /// A consultation opens on its note. A stored one shows its guidance beside it when
    /// there is any; one just recorded shows the transcript, which is complete while the
    ...
    // Closed, the reader goes back to the guidelines it came from, not the first tab
    ...
    // Beside the note the transcript ends level with the note column, never under half
    // the area while the note is still short; in the single selector it has the area
    private void Fit()
    {
