...
public class ModelsContractTest
{
    ...
    {
        await using var engine = EngineProcess.Start($"LOCAL\\clinicavt-models-{Guid.NewGuid():N}");
    ...
    public async Task AStagedManifestIsListedWithItsRole()
    {
        ...
        try
        {
            ...
            Assert.Equal("silero-vad", models[0].GetProperty("name").GetString());  // without a display name the name is the id
