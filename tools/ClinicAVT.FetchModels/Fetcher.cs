...
public sealed class Fetcher(HttpClient http, Action<string> log)
{
    ...
    public async Task<bool> InstallAsync(Pack pack, string root, CancellationToken ct = default)
    {
        ...
        {
            // Sizes let the engine check presence and size at load time without re-hashing
        ...
        string id, CancellationToken ct)
    {
        ...
        {
            from = 0;  // the server ignored the range, so start over
        }
    ...
    // Each shard is appended to the staged file and then deleted, so the extra
    // disk needed at any moment is one shard. The staged length always sits on
    // a shard boundary, which makes an interrupted run resumable.
